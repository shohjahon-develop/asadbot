...
#         button_text = f"{full_name} - {position}"
...
from telegram import Update, ReplyKeyboardMarkup, KeyboardButton, InlineKeyboardButton, InlineKeyboardMarkup, \
    ReplyKeyboardRemove
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, filters, ContextTypes
from config import BOT_TOKEN, POSITIONS, PROXY_URL, PROXY_USERNAME, PROXY_PASSWORD, ADMIN_CHAT_ID, MEDIA_CHANNEL_ID, \
    WELCOME_VIDEO_ID, ADMIN_PASSWORD
from questions import QUESTIONS
from excel_generator import create_excel_file
import httpx
...
async def show_admin_users_page(update: Update, context: ContextTypes.DEFAULT_TYPE, admin_id: int, users_data: dict,
                                page: int = 0):
    """
    Admin paneliga foydalanuvchilarni sahifalar bo'yicha ko'rsatish
    """
...
async def finish_survey(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    # Use variables to avoid f-string expression escaping issues
    position_val = session['data'].get('position', "Noma'lum")
    full_name_val = session['data'].get('full_name', "Noma'lum")
    phone_val = session['data'].get('phone', "Noma'lum")

    logger.info(f"Yangi ariza qabul qilindi:")
    logger.info(f"Lavozim: {position_val}")
    logger.info(f"Ism: {full_name_val}")
    ...
    for voice_msg in session['voice_messages']:
        try:
            caption_text = f"🎤 {full_name_val} - {voice_msg['question_label']}"
            sent_msg = await context.bot.send_voice(
                chat_id=MEDIA_CHANNEL_ID,
                voice=voice_msg['file_id'],
    ...
    for video_msg in session['video_messages']:
        try:
            caption_text = f"🎥 {full_name_val} - {video_msg['question_label']}"
            sent_msg = await context.bot.send_video(
                chat_id=MEDIA_CHANNEL_ID,
                video=video_msg['file_id'],
                caption=caption_text
